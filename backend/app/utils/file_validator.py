...
def validate_mime_type(mime_type: str) -> str:
    """Check MIME type against allowlist. Returns internal file type category."""
...
def validate_file_extension(filename: str) -> None:
    ...
    if ext in BLOCKED_EXTENSIONS:
        raise FileValidationError(f"File extension '{ext}' is blocked for security reasons.")
...
def sanitize_filename(original_filename: str) -> tuple[str, str]:
    """Generate safe stored filename (UUID-based) while preserving cleaned original."""
...
def validate_upload(
    filename: str, mime_type: str, file_size: int, file_data: BinaryIO | None = None,
) -> tuple[str, str, str]:
    """Run all validation checks. Returns (stored_filename, cleaned_name, file_type)."""
    validate_file_extension(filename)
    validate_file_size(file_size)
    file_type = validate_mime_type(mime_type)
    if file_data is not None and not verify_magic_bytes(file_data, mime_type):
        raise FileValidationError("File content does not match the claimed file type.")
